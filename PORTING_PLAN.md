...
**既有輸入/history 邊界與下一個 seam：** 文字/password、checkbox、button/Enter 表單、同視窗文件替換、fragment、page scroll/wheel、session history 與 tabs 均已納入目前 window slice。Bookmark、新視窗與 mailto 外部啟動不在本切片；地址列低寬度可用性、Python 的橘色按鈕／書籤／HTTPS 鎖頭與 native 的視覺差異仍待處理。舊 `tai_present_window` 與 `tai_present_window_with_history` 仍提供不帶可見 chrome 的相容入口。
