...
`tai-browser --window URL` 現在進入 tabbed window，提供可編輯地址列、Back/Forward、URL history、New Tab、Tab N selection，以及作用於 active tab 的 page click、表單輸入/提交、fragment 與 scroll。tab documents 和 CSS 在 loader thread 建構，SDL owner 持續處理輸入、resize 與 close。原有單頁 chrome APIs 仍保留供相容呼叫。本切片維持 `VALIDATING`；800px 真實視窗已驗證 25 個等寬 tabs 的首、中、末格點擊及跨一般／compact 閾值 resize，隔離 2× Wayland 探針已驗證視窗座標到實體像素的分頁點擊。兩分頁 120px 換行標籤／工具列重疊已依 Python 修正；隔離 Xvfb X11 已驗證地址列鍵盤事件順序與導覽；極窄 25 格可讀性、完整 native chrome pixel parity 與 WSLg／Wayland compositor 焦點下的鍵盤交付仍待補足。loader thread 配置故障掃描發現 QuickJS-NG `JS_NewContextRaw()` OOM use-after-free（見 JavaScript 列），專案端清理在修補後的掃描中通過。SDL dummy integration 已在 delayed document/CSS 載入中注入 tab 操作及 close；TabSet tests 另覆蓋 resize、failure routes、Referer 與 late completion。精確幾何、viewport、事件路由與呈現契約見 [presentation contract](docs/reference-presentation.md)；page/session、SDL 資源與 callback ownership 見 [native runtime](docs/architecture/native-runtime.md)。整體驗收仍未完成。
...
**既有輸入/history 邊界與下一個 seam：** 文字/password、checkbox、button/Enter 表單、同視窗文件替換、fragment、page scroll/wheel、session history 與 tabs 均已納入目前 window slice。Bookmark、新視窗與 mailto 外部啟動不在本切片；地址列低寬度可用性、Python 的橘色按鈕／書籤／HTTPS 鎖頭與 native 的視覺差異及 WSLg／Wayland compositor 焦點下的鍵盤交付仍待處理。舊 `tai_present_window` 與 `tai_present_window_with_history` 仍提供不帶可見 chrome 的相容入口。
...
| JavaScript / events | JS runtime/context → `src/js.c` | QuickJS-NG, DOM, CSS, network | VALIDATING | bridge, cancellation, exception tests; `quickjs_oom` and `tabset_loader_oom` allocation failure sweeps | bubbling, broad DOM mutation, timers/fetch, browser integration; QuickJS-NG `JS_NewContextRaw()` frees a context still linked in the GC list when `class_proto` allocation fails (upstream `19dbe85` unchanged), so a later `JS_FreeRuntime()` reads freed memory — fixed by [`patches/quickjs/0001`](patches/quickjs/0001-unlink-context-on-class-proto-oom.patch) until upstream carries it |
