...
`tai-browser --window URL` 以 `TaiPage` 開啟可調整大小的 SDL3 視窗，原生外框初始為
800×600px，尺寸包含 toolbar。Cairo 先在不透明白底繪製 native-endian premultiplied ARGB32；
...
Native `--window` 的 toolbar bottom/address y 依視窗寬度分段：≥232px 為 68.34/49.072，
128–231px 為 82/62.732，79–127px 為 112/92.732，<79px 為 142/122.732。這些 transition
與 Python `Chrome` probe 相同。Page viewport 高度為 `max(1, window_height - bottom)`；
800×600 外框因此使用 800×531.66 page viewport，raster height 向上取整為 532px，並在
toolbar 下方以 y=68.34 呈現。Headless `--screenshot` 仍是 page-only 800×532，不包含 toolbar。
...
Page layout、page raster 和 scrollbar geometry 均使用內容區高度；scrollbar thumb 在內容區
計算後，呈現時加回 toolbar y origin。

初次取得的 physical pixel size 與寬、高皆大於 10px 的 `SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED` 都先以新的
內容區尺寸原子地重建 page layout/display list、夾住 page scroll，然後 raster、upload、present
...
每次 texture 呈現（含 expose）均依當前 `TaiPage` content viewport、clamped scroll 與 max scroll，
